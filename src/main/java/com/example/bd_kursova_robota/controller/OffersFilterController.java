...
public class OffersFilterController extends TableFilterController{

    @FXML ChoiceBox<Object> department;
    @FXML ChoiceBox<Object> specialty;
    @FXML ChoiceBox<Object> educationForm;
    @FXML TextField priceFrom;
    @FXML TextField priceTo;
    ...
    public void onApply(){
        ...
        if (!priceTo.getText().isEmpty() && !priceFrom.getText().isEmpty()){
            try {
                whereClauses.add(String.format("contract_price BETWEEN %1$d AND %2$d", Integer.parseInt(priceFrom.getText()), Integer.parseInt(priceTo.getText())));
            }
            catch (Exception e){
                clearBetween();
            }
